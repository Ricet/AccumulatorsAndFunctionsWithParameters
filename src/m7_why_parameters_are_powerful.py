...
# ----------------------------------------------------------------------
def even_better_draw_circles(r, n, color):
    """
    -- Constructs a window whose width and height are both 400.
    -- Constructs and draws n rg.Circle objects such that:
         -- Each is centered at (200, 200)
         -- They have radii:  0  r  2r  3r  4r ... 20r, respectively.
         -- The outline color is color
         -- Pauses 0.05 seconds after rendering each.
    -- Waits for the user to press the mouse, then closes the window.
    """
    window = rg.RoseWindow(400, 400)
...
def run_test_even_better_draw_circles():
    ...
    print('Testing  draw_circles:  See graphics window')
    print('--------------------------------------------------')
    even_better_draw_circles(5, 30, 'blue')
    even_better_draw_circles(10, 10, 'red')
    even_better_draw_circles(15, 12, 'brown')
